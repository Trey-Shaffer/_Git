Param(	#See general comments at end of file.									`
		#Input file holding DNs of search roots									`
		[string]$SearchRoot =	'D:\PS\ActiveDirectory\ListOuContent\'		+	`
								'InputFile.txt'								,	`
		#Filter used to select desired content.									`
		#  Examples: Person, organizationalUnit									`
		[string]$Filter = 'organizationalUnit'								,	`
		#Output file to hold items retreived.									`
		[string]$OutputFile =	'D:\PS\ActiveDirectory\ListOuContent\'		+	`
								'OutputFile.txt'							,	`
		#Search scope used with SearchBase from input file.						`
		[ValidateSet( 'Base','OneLevel','SubTree' ) ]							`
		[string]$SearchScope = 'SubTree'									,	`
		#Use Domain Controller													`
		[string]$DC = 'AD01'												,	`
		#Display string for banner												`
		[string]$VersionInfo = 'List OU Content 1.1, June 2018'				,	`
		#Root for log file name, with possible date/time addition				`
		[string]$LogBase = 'List'											,	`
		#Path to folder holding script log file.  '' = script folder			`
		[string]$LogFolder = ''												,	`
		#Version number															`
		[string]$Version = '1.1.20181007'									,	`
		#
...
$intCountAllBaseG = 0
$colTargetsG      = Get-InputTextFile_ $Script:SearchRoot
$strFilterG       = 'ObjectClass -eq "'	+	`
					$Script:Filter		+	`
					'"'

' '	| Out-File	-FilePath $Script:OutputFile	`
				-Encoding 'ASCII'				`
				-Force
$strMsgG =	"  Searching for items of type: $Script:Filter"
Write-Both_ $strMsgG

ForEach( $strSearchBaseG IN $colTargetsG )
{	$strMsgG =	"  Processing container: " + $strSearchBaseG
	Write-Both_ $strMsgG
	
	$colObjectsG = Get-ADObject	-Filter        $strFilterG			`
								-SearchBase    $strSearchBaseG		`
								-SearchScope   $Script:SearchScope	`
								-Server        $Script:DC
	
	$intCountThisBaseG = $colObjectsG.Count
	$intCountAllBaseG += $intCountThisBaseG
	$strMsgG =	"`r`n    Found "			+	`
				$intCountThisBaseG			+	`
				" Items`r`n"
	Write-Both_ $strMsgG
	
	ForEach( $oOuG IN $colObjectsG )
	{	$oOuG.Name | Out-File	-FilePath $Script:OutputFile	`
								-Encoding 'ASCII'				`
								-Append
	}#Next - oOuG
}#Next - SearchBase

$strMsgG =	"`r`n`r`n  Found "	+	`
			$intCountAllBaseG	+	`
			' Items total.'
Write-Both_ $strMsgG
